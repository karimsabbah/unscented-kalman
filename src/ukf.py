...
class SigmaWeights:
    ...
    def computeWeights(self):
        ...
        self.Wc[0] = self._lambda / (self.n + self._lambda) + (1. - alpha**2 +
                                                               beta)
...
def unscentedTransform(sigmas, sigmasState, Q):
    ...
    for i in range(2 * sigmas.n + 1):
        temp = (sigmasState[i].reshape((sigmas.n, 1)) - mean.reshape(
            (sigmas.n, 1)))
...
def computeCrossCovariance_Pxz(sigmas, meanPredict, meanZ, sigmasState,
                               sigmasZ):

    lenColZ = sigmasZ.shape[1]
    covariance = np.zeros((sigmas.n, lenColZ))
    for i in range(2 * sigmas.n + 1):
        temp1 = (sigmasState[i].reshape((sigmas.n, 1)) - meanPredict.reshape(
            (sigmas.n, 1)))
        temp2 = (sigmasZ[i].reshape((lenColZ, 1)) - meanZ.reshape(
            (lenColZ, 1)))
...
def Q(t):
    t = t % (60 / HR)
    S = 0.065
    return (1.4 / (S * np.sqrt(np.pi))) * np.exp(-((t - 0.36)**2) /
                                                 (2 * S**2)) + 5
...
if __name__ == "__main__":
    ...
    HR = 80
    cycle_time = 60 / HR
    time = np.linspace(0, (60 / HR) * cycles + dt,
                       num=((60 / HR) * cycles + dt) / dt)
    time_noise = np.linspace(0, (60 / HR) * 5 + dt,
                             num=((60 / HR) * 5 + dt) / dt)
    y0 = 1
    solNoise, zNoise = mkNoisyMeasurements(f, y0, time_noise, z_std,
                                           std_constrain)
    ...
    for i in range(len(time)):
        ...
        sigma_point_locations = np.array([[sigmas.sigmaPoints[:, 0]],
                                          [sigmas.sigmaPoints[:, 1]]])
        #        plt.plot(sigma_point_locations[0,:],sigma_point_locations[1,:],'o')
        sigmasState = stateTransition(sigmas, f, bitch_time, dt)
        meanPredict, covPredict = unscentedTransform(sigmas, sigmasState,
                                                     Qprocess)
        ...
        crossCov = computeCrossCovariance_Pxz(sigmas, meanPredict, meanZ,
                                              sigmasState, sigmasZ)
